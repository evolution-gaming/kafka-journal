...
private[replicator] final class AsyncPeriodicTopicCommit[F[_]: MonadThrowable] private[commit] (
  ...
  override def apply(offsets: NonEmptyMap[Partition, Offset]): F[Unit] = {
    stateRef.update(_.applyMarkedForAssigned(offsets.toSortedMap))
  }
    ...
    for {
      ...
      _ <-
        if (anythingCommitted) {
          stateRef.update(_.applyCommittedForAssigned(offsetsToCommit)) >>
...
private[replicator] object AsyncPeriodicTopicCommit {
  ...
  private[commit] final case class State(
    ...
    def applyCommittedForAssigned(offsets: Iterable[(Partition, Offset)]): State = {
      applyForAssigned(offsets)(_.updateCommittedIfNeeded(_))
    }

    def applyMarkedForAssigned(offsets: Iterable[(Partition, Offset)]): State = {
      applyForAssigned(offsets)(_.updateMarkIfNeeded(_))
    }
    ...
    private def applyForAssigned(offsets: Iterable[(Partition, Offset)])(f: (PartitionState, Offset) => PartitionState): State = {
      copy(
  ...
  private[commit] final case class PartitionState(
    ...
    def updateMarkIfNeeded(newMarkedOffset: Offset): PartitionState = {
