...
class AsyncPeriodicTopicCommitStateTest extends AnyFreeSpec with Matchers {
    ...
    "when updating marked offset" - {
      "updates if new value is higher" in {
        pState(
          marked    = 2.some,
          committed = 1.some,
        ).updateMarkIfNeeded(offset(3)) shouldEqual
          pState(
            marked    = 3.some,
            committed = 1.some,
          )
      ...
      "ignore update if the new value is not higher" in {
        pState(
          marked    = 2.some,
          committed = 1.some,
        ).updateMarkIfNeeded(offset(1)) shouldEqual
          pState(
            marked    = 2.some,
            committed = 1.some,
          )
  ...
  "State" - {
    "has something to commit when some partitions have uncommitted progress" in {
      ...
      state = state.applyCommittedForAssigned(
        Vector(
          p(0) -> offset(1),
          p(1) -> offset(11),
          p(2) -> offset(21),
      ...
      state = state.applyMarkedForAssigned(
        Vector(
          p(0) -> offset(2),
          p(1) -> offset(11),
          p(2) -> offset(22),
    ...
    "ignores updates on non-assigned partitions" - {
      ...
      "when committed offsets updated" in {
        ...
        state.applyCommittedForAssigned(
          Vector(
            p(0) -> offset(1),
            p(1) -> offset(2),
            p(2) -> offset(3),
