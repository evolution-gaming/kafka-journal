...
private[journal] object Batch_4_1_0_Alternative_with_Vector {

  def of(records: NonEmptyList[ActionRecord[Action]]): List[Batch_4_1_0_Alternative_with_Vector] = {

    val actions = records
      .foldLeft(Vector.empty[ActionRecord[Action]]) { // CHANGE: use `Vector`
        ...
        case (acc, record) => acc :+ record // CHANGE: use `prepend`
      }
      .groupBy { record =>
        record.action match {
