...
trait EventualJournalSpec extends AnyWordSpec with Matchers {
  ...
  def test[F[_]: BracketThrowable: Fail](
          ...
          withJournals2 {
            case (eventual, replicated) =>
              ...
              for {
                ...
                a <- eventual.events()
              } yield a shouldEqual events ++ head
          }
        }
          ...
          withJournals2 {
            case (eventual, replicated) =>
              for {
                ...
                a       <- eventual.pointer
              } yield a shouldEqual pointerLast.map(_.withPartitionOffset(partitionOffsetNext))
          }
        }
          ...
          withJournals2 {
            case (eventual, replicated) =>
              for {
                ...
                a       <- eventual.pointer
              } yield a shouldEqual event.pointer.some
          }
        }
      }
          ...
          withJournals2 {
            case (eventual, replicated) =>
              for {
                ...
                _                = a shouldEqual Nil
                a               <- eventual.pointer
              } yield a shouldEqual JournalPointer(partitionOffset2, deleteTo.value).some
          }
        }
      ...
      s"pointer, $name" in {
        withJournals2 {
          case (eventual, _) =>
            for {
              a <- eventual.pointer
            } yield a shouldEqual pointerLast
        }
      }
    }
        ...
        s"save pointer, $name, topic: $topic" in {
          withJournals3 {
            case (_, replicated) =>
              ...
              } yield a shouldEqual offset.some
          }
        }
      ...
      s"topics, $name" in {
        withJournals3 {
          case (_, replicated) =>
            for {
              a <- replicated.topics
            } yield a shouldEqual topics.toSortedSet
        }
      }
        ...
        withJournals1 {
          case (eventual, replicated) =>
            for {
              ...
              _  = a shouldEqual Nil
              a <- eventual.pointer
            } yield a shouldEqual deleteToPointer.some
        }
      }
      ...
      withJournals1 {
        case (eventual, replicated) =>
          for {
            ...
            _      = a shouldEqual Nil
            a     <- eventual.pointer
          } yield a shouldEqual journalPointerOf(offset = 8, seqNr = event3.seqNr).some
      }
    }
    ...
    "append not from beginning" in {
      ...
      withJournals1 {
        case (eventual, replicated) =>
          ...
          } yield a shouldEqual events.toList
      }
    }
    ...
    "ids" in {
      withJournals1 {
        case (eventual, replicated) =>
          ...
          } yield ids shouldEqual List.empty
      }
    }
  }
}
