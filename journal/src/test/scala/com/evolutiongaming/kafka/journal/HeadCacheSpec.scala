      ...
      val result = for {
        ...
        _ <- headCacheOf(eventual, consumer).use { headCache =>
          for {
            ...
            state  <- stateRef.get
          } yield state shouldEqual TestConsumer.State(
            actions = List(
...
object HeadCacheSpec {
  ...
  def consumerRecordOf(action: Action, topicPartition: TopicPartition, offset: Offset)(implicit
    actionToProducerRecord: ActionToProducerRecord[Try],
  ): ConsumerRecord[String, Unit] =
    ConsumerRecordOf[Try](action, topicPartition, offset).get.void
    ...
    def apply(stateRef: Ref[IO, State]): TopicCache.Consumer[IO] =
      new TopicCache.Consumer[IO] {
        ...
        def partitions(topic: Topic) =
          for {
            state <- stateRef.get
          } yield state.topics.get(topic).fold(Set.empty[Partition])(_.toSet)
      }
