...
/** Configuration of Kafka client used to store the journal.
  *
  * The configuration uses standard consumer and producer configuration from skafka. It is highly recommended to keep
  * the default settings unless some internal issue is to be worked around as the logic of persistence plugin may depend
