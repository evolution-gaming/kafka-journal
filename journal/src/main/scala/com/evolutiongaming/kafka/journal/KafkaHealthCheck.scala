...
object KafkaHealthCheck {
  ...
  def of[F[_]: Temporal](
    ...
    val result = for {
      ...
      fiber <- (producer, consumer).tupled.use {
        case (producer, consumer) => run(key, config, stop, producer, consumer, ref.set, log)
      }.start
    } yield {
  ...
  object Producer {
    ...
    def of[F[_]: Monad: KafkaProducerOf: FromTry](topic: Topic, config: ProducerConfig): Resource[F, Producer[F]] =
      ...
      } yield Producer[F](topic = topic, producer = producer)
  }
    ...
    def apply[F[_]: Functor](consumer: KafkaConsumer[F, String, String]): Consumer[F] =
      new Consumer[F] {
        ...
        def poll(timeout: FiniteDuration) =
          ...
          } yield Record(key = record.key.map(_.value), value = record.value.map(_.value))
      }
