...
import cats.{Monad, MonadThrow, Parallel, Semigroup}
...
object PartitionCache {
  ...
  def of[F[_]: Concurrent: Runtime: Parallel: Timer](
    maxSize: Int = 10000,
  ...
  private def main[F[_]: Concurrent: Runtime: Parallel: Timer](
    maxSize: Int,
                ...
                for {
                  ...
                  complete = (result: Result.Now) => {
                    deferred
                      ...
                      .attempt
                      .flatMap {
                        case Right(_) => cache.remove(key).void
                        case Left(_)  => ().pure[F]
                      }
                  }
                  _ <- Timer[F]
