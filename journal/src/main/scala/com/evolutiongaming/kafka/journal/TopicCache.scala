 ...
 *
 * The implementation reads both Kafka and Cassandra by itself, continously refreshing the
 * information.
 */
...
private[journal] object TopicCache {
   ...
   *   Function used to parse records coming from `consumer`. Only headers will be parsed, and the
   ...
   *
   * Allows easier stubbing in unit tests and provides a little bit more convenient
   * [[TopicCache]]-specific API.
   */
  trait Consumer[F[_]] {
     ...
     *
     * The read will start from the new offsets the next time [[KafkaConsumer#poll]] is called.
     *
     * @see
  ...
  object Consumer {
     ...
     *
     * @param config
     *   Kafka configuration in form of [[ConsumerConfig]]. It is used to get Kafka address, mostly,
     *   and some important parameters will be ignored, as these need to be set to specific values
     *   for the cache to work. I.e. `autoOffsetReset`, `groupId` and `autoCommit` will not be used.
     * @param pollTimeout
     *   The timeout to use for [[KafkaConsumer#poll]].
     */
   ...
   *
   * If one has to calcuate an average for a large list of numbers, one does not have to keep all
   * these numbers in a memory. It is enough to keep sum of them and the count.
   *
   * @param sum
  ...
  private implicit final class SetOps[A](val self: Set[A]) extends AnyVal {
     ...
     *
     * It is possible to achieve the same using `alleycats-core` library like this, so the method
  ...
  private implicit final class MapOps[K, V](val self: Map[K, V]) extends AnyVal {
