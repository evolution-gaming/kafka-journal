...
object Journals {
                  ...
                  def read(seqNr: SeqNr) =
                    for {
                      ...
                      readKafka = (seqNr: SeqNr, offset0: Option[Offset]) => {
                        ...
                        } yield EventRecord(action, event, record.partitionOffset, events.metadata)
                        records.stateful(seqNr) {
                          case (seqNr, a) =>
                            if (seqNr <= a.seqNr) {
                              val seqNr1 = a.seqNr
                                .next[Option]
  ...
  object Producer {
    ...
    def apply[F[_]: Monad: Fail](
    ...
    ): Producer[F] = { (record: ProducerRecord[String, ByteVector]) =>
      for {
      ...
      } yield PartitionOffset(partition, offset)
    }
  }
  ...
  object Consumer {
    ...
    def of[F[_]: MonadThrowable: KafkaConsumerOf: FromTry](
      ...
      } yield apply[F](kafkaConsumer, pollTimeout)
    }
  ...
  implicit class JournalsOps[F[_]](val self: Journals[F]) extends AnyVal {
    ...
    def withLog(log: Log[F], config: Journal.CallTimeThresholds = Journal.CallTimeThresholds.default)(implicit
      F: FlatMap[F],
    ...
    def withLogError(log: Log[F])(implicit
      F: MonadThrowable[F],
      measureDuration: MeasureDuration[F],
    ): Journals[F] = { (key: Key) =>
      self(key).withLogError(key, log)
    }

    def withMetrics(metrics: JournalMetrics[F])(implicit
      F: MonadThrowable[F],
      measureDuration: MeasureDuration[F],
    ): Journals[F] = { (key: Key) =>
    ...
    def mapK[G[_]](fg: F ~> G, gf: G ~> F): Journals[G] = { (key: Key) =>
      self(key).mapK(fg, gf)
    }
  }
}
