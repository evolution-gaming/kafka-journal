 ...
 *
 * The usual practice is that [[PayloadAndType#payload]] will be used as a value for a Kafka record,
 * and [[PayloadAndType#payloadType]] will get into a header.
 *
 * @param payload
 *   Used to store actual payload, i.e. one or several journal events. The contents are usually a
 *   [[PayloadJson]] instance serialized into a JSON form.
 * @param payloadType
 *   Used to determine how the contents of `payload` should be treated, i.e. if it should be parsed
 *   as JSON.
 */
...
object PayloadAndType {
   ...
   *
   * It represents two fields from [[EventJson]] class, i.e. [[EventJson#payload]] and
   * [[EventJson#payloadType]].
   *
   * The reason this class was created is to make the logic constructing [[EventJson]] in
   * [[conversions.KafkaWrite#writeJson]] and [[conversions.KafkaRead#readJson]] generic by having
   * ability to convert from an actual payload to [[EventJsonPayloadAndType]] and back.
   *
   ...
   *
   * It usually corresponds to a single `Payload` instance if Play JSON is used, or `Json` instance
   * if Circe is used instead.
   *
   * @param payload
   *   Serialized payload in JSON or String form.
   * @param payloadType
  ...
  object PayloadJson {
    ...
    implicit def codecPayloadJson(
      implicit
      jsonCodec: JsonCodec[Try],
    ): Codec[PayloadJson[JsValue]] = formatCodec // TODO not used
