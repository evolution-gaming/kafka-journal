...
/** Metainfo of events written to Kafka, but not yet replicated to Cassandra.
  *
  * The implementation subscribes to all events in Kafka and periodically polls Cassandra to remove information about
  * the events, which already replicated.
  *
  * The returned entries do not contain the events themselves, but only an offset of the first non-replicated event, the
  * sequence number of last event, range of events to be deleted etc.
  *
  * The consuming/polling of the records will stop after a configured timeout (i.e. [[HeadCacheConfig#expiry]]) for the
  * topics where no activity / recoveries happen. It will restart when the new calls to a cache come for these topics
  * again.
  *
  * TODO headcache:
  *   1. Keep 1000 last seen entries, even if replicated. 2. Fail headcache when background tasks failed
  *
  * @see
...
trait HeadCache[F[_]] {
  ...
  /** Get the information about a state of a journal stored in the cache.
    ...
    *
    * @return
    *   [[HeadInfo]] with the current metainformation about non-replicated events, or `None` if it was not present in
    *   [[HeadCache]] and could not be loaded either.
    */
...
object HeadCache {
  ...
  /** Creates new cache using a Kafka configuration and Cassandra reader.
    *
    * The created instances will report metrics to `metrics` and also will do the debug logging. There is no need to
    * call [[HeadCache#withLogs]] on them.
    *
    * @param consumerConfig
    *   Kafka consumer configuration used to find new non-replicated journal events. Some of the parameters will be
    *   ignored. See [[TopicCache.Consumer#of]] for more details.
    * @param eventualJournal
    *   Cassandra (or other long term storage) data source used to remove replicated events from the cache. Usually
    *   created by calling [[EventualCassandra#of]].
    * @param metrics
    *   Interface to report the metrics to. The intended way to configure it is overriding [[KafkaJournal#metrics]] in a
    *   custom implementation of [[KafkaJournal]].
    * @return
    *   Resource which will configure a [[HeadCache]] with the passed parameters. Instance of `Resource[HeadCache]` are,
    *   obviously, reusable and there is no need to call [[HeadCache#of]] each time if parameters did not change.
    */
  def of[F[
    _,
  ]: Async: Parallel: Runtime: LogOf: KafkaConsumerOf: MeasureDuration: FromTry: FromJsResult: JsonCodec.Decode](
    ...
    } yield result.withLog(log)

  /** Creates new cache using Kafka and Cassandra data sources.
    *
    * The method also allows to change the default configuration in form of [[HeadCacheConfig]], i.e. to make the
    * polling faster for testing purposes.
    *
    * @param eventual
    *   Cassandra data source.
    * @param log
    *   Logger to use for [[TopicCache#withLog]]. Note, that only [[TopicCache]] debug logging will be affected by this.
    *   One needs to call [[HeadCache#withLog]] if debug logging for [[HeadCache]] is required.
    * @param consumer
    *   Kakfa data source factory. The reason why it is factory (i.e. `Resource`) is that [[HeadCache]] will try to
    *   recreate consumer in case of the failure.
    * @param metrics
    *   Interface to report the metrics to. The intended way to configure it is overriding [[KafkaJournal#metrics]] in a
    *   custom implementation of [[KafkaJournal]].
    * @param config
    *   Cache configuration. It is recommended to keep it default, and only change it for unit testing purposes.
    * @return
    *   Resource which will configure a [[HeadCache]] with the passed parameters. Instance of `Resource[HeadCache]` are,
    *   obviously, reusable and there is no need to call [[HeadCache#of]] each time if parameters did not change.
    */
  ...
  sealed abstract private class WithFence
  ...
  implicit class HeadCacheOps[F[_]](val self: HeadCache[F]) extends AnyVal {
    ...
    /** Prevents cache methods to be used after `Resource` was released.
      ...
      *
      * It may prevent certain kind of bugs, when several caches are, accidentially, alive and working.
      */
  ...
  trait Metrics[F[_]] {
    ...
    /** Report health of all [[PartitionCache]] instances related to a topic.
      ...
      *   [[HeadCacheConfig.Partition#maxSize]] multiplied by number of partitions, the cache might not work
      *   efficiently.
      * @param listeners
    ...
    /** Report the latency and number of records coming from Kafka.
      *
      * I.e. how long it took for a next element in a stream returned by [[HeadCacheConsumption#apply]] to get from a
      * journal writer to this cache.
      *
      * @param topic
      ...
      * @param diff
      *   The number of elements added to cache by this batch, i.e. returned by [[PartitionCache#add]].
      */
    ...
    /** Report the number of records coming from Cassandra.
      ...
      * @param diff
      *   The number of elements remove from cache by this batch, i.e. returned by [[PartitionCache#remove]].
      */
  ...
  object Metrics {
    ...
    /** Registers a default set of metrics to a passed collector registry.
      *
      * Note, that creating this metrics several times with the same collector registry may cause errors unless previous
      * [[Metrics]] instance was not released yet.
      *
      * The following metrics will be registered by default, but it is possible to override the default `headcache`
