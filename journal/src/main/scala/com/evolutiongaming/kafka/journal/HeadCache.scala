 ...
 *
 * The implementation subscribes to all events in Kafka and periodically polls Cassandra to remove
 * information about the events, which already replicated.
 *
 * The returned entries do not contain the events themselves, but only an offset of the first
 * non-replicated event, the sequence number of last event, range of events to be deleted etc.
 *
 * The consuming/polling of the records will stop after a configured timeout (i.e.
 * [[HeadCacheConfig#expiry]]) for the topics where no activity / recoveries happen. It will restart
 * when the new calls to a cache come for these topics again.
 *
 * TODO headcache:
 ...
 *
 * @see
 *   [[HeadInfo]] for more details on the purpose of the stored data.
 */
trait HeadCache[F[_]] {
   ...
   *
   * @param key
   *   Journal key including a Kafka topic where journal is stored and a journal identifier.
   * @param partition
   *   Partition where journal is stored to. The usual way to get the partition is to write a
   *   "marker" record to Kafka topic and use the partition of the marker as a current one.
   * @param offset
   ...
   *
   * @return
   *   [[HeadInfo]] with the current metainformation about non-replicated events, or `None` if it
   *   was not present in [[HeadCache]] and could not be loaded either.
   */
...
object HeadCache {
   ...
   *
   * The created instances will report metrics to `metrics` and also will do the debug logging.
   * There is no need to call [[HeadCache#withLogs]] on them.
   *
   * @param consumerConfig
   *   Kafka consumer configuration used to find new non-replicated journal events. Some of the
   *   parameters will be ignored. See [[TopicCache.Consumer#of]] for more details.
   * @param eventualJournal
   *   Cassandra (or other long term storage) data source used to remove replicated events from the
   *   cache. Usually created by calling [[EventualCassandra#of]].
   * @param metrics
   *   Interface to report the metrics to. The intended way to configure it is overriding
   *   [[KafkaJournal#metrics]] in a custom implementation of [[KafkaJournal]].
   * @return
   *   Resource which will configure a [[HeadCache]] with the passed parameters. Instance of
   *   `Resource[HeadCache]` are, obviously, reusable and there is no need to call [[HeadCache#of]]
   *   each time if parameters did not change.
   */
  def make[
   ...
   *
   * The method also allows to change the default configuration in form of [[HeadCacheConfig]], i.e.
   * to make the polling faster for testing purposes.
   *
   * @param eventual
   *   Cassandra data source.
   * @param log
   *   Logger to use for [[TopicCache#withLog]]. Note, that only [[TopicCache]] debug logging will
   *   be affected by this. One needs to call [[HeadCache#withLog]] if debug logging for
   *   [[HeadCache]] is required.
   * @param consumer
   *   Kafka data source factory. The reason why it is factory (i.e. `Resource`) is that
   *   [[HeadCache]] will try to recreate consumer in case of the failure.
   * @param metrics
   *   Interface to report the metrics to. The intended way to configure it is overriding
   *   [[KafkaJournal#metrics]] in a custom implementation of [[KafkaJournal]].
   * @param config
   *   Cache configuration. It is recommended to keep it default, and only change it for unit
   *   testing purposes.
   * @return
   *   Resource which will configure a [[HeadCache]] with the passed parameters. Instance of
   *   `Resource[HeadCache]` are, obviously, reusable and there is no need to call [[HeadCache#of]]
  ...
  trait Eventual[F[_]] {
     ...
     *
     * @see
     *   [[EventualJournal#offset]] for more details.
     */
  ...
  private sealed abstract class WithFence
  ...
  implicit class HeadCacheOps[F[_]](val self: HeadCache[F]) extends AnyVal {
     ...
     *
     * The messages will go to DEBUG level, so it is also necessary to enable it in logger
     * configuration.
     */
    def withLog(
      log: Log[F],
     ...
     *
     * [[ReleasedError]] will be raised if [[HeadCache#get]] is called after resource is released.
     *
     * It may prevent certain kind of bugs, when several caches are, accidentally, alive and
     * working.
     */
    def withFence(
      implicit
  ...
  trait Metrics[F[_]] {
     ...
     * @param now
     *   If result was [[PartitionCache.Result.Now]], i.e. entry was already in cache.
     */
    def get(
      topic: Topic,
     ...
     * @param entries
     *   Number of distinct journals stored in a topic cache. If it is too close to
     *   [[HeadCacheConfig.Partition#maxSize]] multiplied by number of partitions, the cache might
     ...
     * [[HeadCacheConsumption#apply]] to get from a journal writer to this cache.
     *
     * @param topic
     ...
     * @param diff
     *   The number of elements remove from cache by this batch, i.e. returned by
     *   [[PartitionCache#remove]].
     */
  ...
  object Metrics {
     ...
     *
     * Note, that creating this metrics several times with the same collector registry may cause
     * errors unless previous [[Metrics]] instance was not released yet.
     *
     * The following metrics will be registered by default, but it is possible to override the
     * default `headcache` prefix to something else.
     *
     * {{{
    ...
    def make[F[_]: Monad](
      ...
      val entriesGauge =
        registry.gauge(name = s"${ prefix }_entries", help = "HeadCache entries", labels = LabelNames("topic"))
