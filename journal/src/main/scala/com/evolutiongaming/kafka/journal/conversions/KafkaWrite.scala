...
/** Prepare payload for storing into Kafka.
  *
  * Converts a structure convenient for a business logic to a structure, which is convenient to store into Kafka.
  */
...
object KafkaWrite {
  ...
  implicit def payloadKafkaWrite[F[_]: MonadThrowable](implicit
    ...
    /** Safely cast an argument to `Event[Payload.TextOrJson]`, or returns `None` if payload is binary instead.
      */
    ...
    /** Casts all list elements to `Event[Payload.TextOrJson]` or returns `None` if list contains events with binary
      * payload,
      *
    ...
    def eventJsons(
      ...
      result: List[Event[Payload.TextOrJson]],
