...
private[journal] object ResourcePool {
   ...
   */
  def fixedSize[F[_]: Concurrent, O](makeObject: Resource[F, O], poolSize: Int): Resource[F, ResourcePool[F, O]] = {
    for {
      objects <- Vector.fill(poolSize)(makeObject).sequence
      unused <- Queue.bounded[F, O](poolSize).toResource
      _ <- objects.traverse_(unused.offer).toResource
    } yield {
      class Main
