...
/** Prevents overload of the expensive resources.
  *
  * The idea is that a single damper is set up to protect a resource, which could be overwhelmed, i.e. a
  * [[KafkaConsumer]].
  *
  * Then, before using a resource, [[#acquire]] is called, and, when resource is not needed anymore, [[#release]] is
  * called instead.
  *
  ...
  *
  * The more complicated implementation may only delay the [[#acquire]] call if sufficient number of calls accumulated,
  * i.e. if there are `< N` ongoing calls then [[#acquire]] returns immediately, or sleeps for some time, otherwise.
  *
  * The recommended way to use the instances of [[Damper]] is to call [[Damper.DamperOps#resource]] to ensure the
  * [[#release]] calls are not forgotten. The explicit calls of [[#acquire]] and [[#release]] are reserved for the cases
...
trait Damper[F[_]] {
  ...
  /** Call before using an expensive resource.
    ...
    *
    * The recommended way to use the instances of [[Damper]] is to call [[Damper.DamperOps#resource]] to ensure the
    * [[#release]] calls are not forgotten. The explicit calls of [[#acquire]] and [[#release]] are reserved for the
...
object Damper {
  ...
  def of[F[_]: Async](delayOf: Acquired => FiniteDuration): F[Damper[F]] = {
    ...
    object State {
      ...
      /** There are some [[#acquire]] calls sleeping.
        ...
        *   Number of [[#acquire]] calls without corresponding [[#release]] calls.
        * @param entries
        *   List of the handles allowing specific [[#acquire]] call to happen. As soon as respective [[Entry]] is
        *   called, the fiber will stop waiting and `acquired` will increment.
        * @param wakeUp
        *   The deferred marks currently ongoing delay. It will be completed by a value from `entries` when the sleep is
        *   over, or `None` if the sleep is interuptted by call of [[#release]].
        */
    ...
    Ref[F]
      ...
      .map { ref =>
        ...
        /** Schedule a next sleep for queued entries.
          ...
          * @param effect
          *   Wake up callback for a first call to be allowed. It could be several `Entry` elements glued together, if
          *   the first one has a delay scheduled, and others have zero delays.
          */
  ...
  implicit class DamperOps[F[_]](val self: Damper[F]) extends AnyVal {
    ...
    /** Converts [[Damper]] to a [[cats.effect.Resource]].
      *
      * This is, actually, a prefered way to use [[Damper]] to ensure [[Damper#release]] is always called after
      * appropriate [[Damper#acquire]].
      */
  ...
  implicit private class DeferredOps[F[_], A](val self: Deferred[F, A]) extends AnyVal {
