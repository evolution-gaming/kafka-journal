...
trait StreamActionRecords[F[_]] {
  ...
  /** Creates a stream of events from Kafka starting with specified offset.
    *
    * @param offset
    *   The offset to start the reading from. If `None` then reading will start from the beginning, which could either
    *   be the begining of Kafka topic partion, or another offset, if the stream implementation limits it to some newer
    *   offset.
    */
...
object StreamActionRecords {
  ...
  /** Creates a reader for events not yet replicated to Cassandra.
    *
    * When [[StreamActionRecords!#apply]] is called, the reader will stream messages starting with passed `offset`
    * parameter, but not older than `offsetReplicated` and `seqNr`, and not later than passed `marker`.
    *
    ...
    * }}}
    *
    * If `offset` parameter passed to [[StreamActionRecords!#apply]] is outside of these bounds, then empty stream will
    * be returned indicating that there is no unreplicated message with such offset.
    *
    * This is a main optimization comparing to what [[ConsumeActionRecords]] does, in addition to filtering out
    * [[Action.System]] records, so previously sent markers etc. are not included.
    *
    * @param key
    *   Journal identifier.
    * @param from
