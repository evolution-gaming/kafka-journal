 ...
 *
 * If journal has a non-deleted events pending a replication, it will contain [[Offset]] of the
 * _first_ (i.e. oldest) event and [[SeqNr]] of the _last_ (i.e. newest) event in a form of
 * [[HeadInfo.Append]] class, with [[HeadInfo.Append#deleteTo]] specifying the part to be deleted,
 * if any.
 *
 * If the journal was purged or all events were deleted, it will be [[HeadInfo.Purge]] or
 * [[HeadInfo.Delete]], accordingly.
 *
 * Having this information (by preparing it using [[HeadCache]]) allows one to avoid re-reading
 * Kafka during recovery, if it is [[HeadInfo.Empty]], or start reading Kafka topic partition much
 * later, by skipping events unrelated to the journal.
 *
 * @see
...
object HeadInfo {
   ...
   *
   * [[HeadInfo#empty]] is equivalent to [[HeadInfo.Empty]], but the inferred type will be
   * `HeadInfo` rather than `HeadInfo.Empty`, which might be a little more convenient in some cases.
   *
   * Example:
   * {{{
   * scala> import com.evolution.kafka.journal.HeadInfo
   *
   ...
   *
   * [[HeadInfo#delete]] is equivalent to [[HeadInfo.Delete]], but the inferred type will be
   * `HeadInfo` rather than `HeadInfo.Delete`, which might be a little more convenient in some
   * cases.
   *
   * Example:
   * {{{
   ...
   *
   * [[HeadInfo#append]] is equivalent to [[HeadInfo.Append]], but the inferred type will be
   * `HeadInfo` rather than `HeadInfo.Append`, which might be a little more convenient in some
   * cases.
   *
   * Example:
   * {{{
   * scala> import com.evolution.kafka.journal.*
   ...
   * There are no non-replicated events in Kafka for specific journal.
   *
   * Having this state means it is safe to assume the journal is fully replicated to a storage (i.e.
   * Cassandra).
   *
   ...
   *
   * Having this state means it is not enough to read Cassandra to get a full journal. One has to
   * read Kafka also.
   */
   ...
   * The only non-replicated records are delete actions.
   *
   * The events itself already replicated to Cassandra, so it should be enough to read them from
   * Cassandra starting from the first non-deleted event, i.e. there is no need to read Kafka in
   * this case.
   *
  ...
  implicit class HeadInfoOps(val self: HeadInfo) extends AnyVal {
     ...
     *
     * We update last saved [[SeqNr]], but ignore [[Offset]] unless this is a first event in our
     * list of non-replicted events.
     */
     ...
     *
     * The marker only matters during recovery, to validate that all journal events were read. There
     * is no, currently, a reason to track if markers made by previous recoveries did replicate.
     */
     ...
     *
     * Note, that no actual event body is required, only [[ActionHeader]] and [[Offset]] of the new
     * record.
     */
