 ...
 *
 * The class itself does not read Kafka or poll Cassandra (or other long term storage), it relies on
 * the information incoming through [[PartitionCache#add]] (for Kafka updates), and
 * [[PartitionCache#remove]] (for Cassandra updates).
 */
private[journal] trait PartitionCache[F[_]] {
   ...
   *   Journal id
   * @param offset
   *   Current [[Offset]], i.e. maximum offset where Kafka records related to a journal are located.
   *   The usual way to get such an offset is to write a "marker" record to Kafka patition and use
   *   the offset of the marker as a current one.
   *
   * @return
   *   [[PartitionCache.Result]] with either the current state or indication of a reason why such
   *   state is not present in a cache.
   *
   * @see
   ...
   *
   * Such a value is useful, because there is no point to read Kafka for new non-replicated events
   * earlier than this offset.
   *
   * I.e. if we seen it in Cassandra, it means the events are already replicated, and if we seen in
   * Kafka, it means we already handled them in our [[#add]] method.
   *
   * @return
   ...
   *
   * The method is intended to be called after the information is received from Kafka. It is
   * possible that part or all of these events are already replicated.
   *
   * @param records
   *   Metainformation of the incoming records.
   * @return
   *   Number of additional records marked as non-replicated after the method was called. `None` if
   *   no new records were counted as non-replicated or if this is the first call on an empty cache.
   */
   ...
   *
   * @param offset
   *   All events with offset less or equal to the parameter are now replicated to Cassandra (or
   *   other long term storage).
   * @return
   *   Number of additional records marked as replicated after the method was called. `None` if such
   *   offset or later was already removed or not present in the cache in the first place.
   */
   ...
   *
   * The call might be relatively expensive if there are a lot of non-replicated journals, so it
   * might be worth to call it outside of the large loops. I.e. call it once a minute to update the
   * metrics.
   *
   * @see
...
private[journal] object PartitionCache {
   ...
   *
   * The parameters are, usually, configured in [[HeadCacheConfig]], but could also be set directly,
   * i.e. for unit testing purposes.
   *
   * @param maxSize
   *   Maximum number of journals to store in the cache.
   * @param dropUponLimit
   *   Proportion of number of journals to drop if `maxSize` is reached. Value outside of the range
   *   of `0.01` to `1.0` will be ignored. `0.01` means that 1% of journals will get dropped, and
   *   `1.0` means that 100% of journals will get dropped.
   * @param timeout
   *   Duration to wait in [[Result.Later]] returned by [[Partition#get]] if entry is not found in a
   *   cache.
   * @return
   *   Resource which will configure a [[PartitionCache]] with the passed parameters. Instance of
   *   `Resource[PartitionCache]` are, obviously, reusable and there is no need to call
   *   [[PartitionCache#of]] each time if parameters did not change.
   */
  def make[F[_]: Async](
    maxSize: Int = 10000,
  ...
  private def main[F[_]: Async](
     ...
     *
     * When [[PartitionCache#get]] cannot find an actual entry for a given journal, it sets up an
     ...
     * @param offset
     *   The last _replicated_ offset seen by [[PartitionCache]] in Cassandra (or another long term
     *   storage).
     * @param entries
  ...
  object Result {
    ...
    object Now {
       ...
       * [[PartitionCache]] has seen given offset in Kafka, but not Cassandra.
       *
       * In other words, the journal is not fully replicated, but we have [[HeadInfo]] in cache to
       ...
       * The timeout occured while waiting for the [[HeadInfo]] value to load.
       *
       * When [[PartitionCache#get]] is called and [[HeadInfo]] is not yet available in the cache,
       * the returned `F[Result]` will contain [[Result.Later]], with value, which will try to wait
       * until either [[PartitionCache#add]] or [[PartitionCache#remove]] will bring the required
       * information into [[PartitionCache]].
       *
       * This avoids reading Kafka in parallel as, usually, the consumer calling these methods on
       * [[PartitionCache]] is already processing these records.
       *
       * Saying that, if consumer is too slow (during Kafka rebalancing?), and the information does
       * not quickly replicate to Cassandra either, then we want to report a timeout allowing the
       * caller to handle the situation by itself.
       *
       * @param duration
     ...
     *
     * The caller may still try to get [[HeadInfo]] without calculating it themselves if
     * [[PartitionCache]] gets the information, before configured timeout kicks in, by calling
     * [[Later#value]].
     *
     * @see
    ...
    implicit class ResultOps[F[_]](val self: Result[F]) extends AnyVal {
       ...
       *
       * Roughly speaking, the only case when a caller may care if [[Now]] was immediately available
       * or had to wait a bit for be loaded is metrics.
       *
   ...
   *
   * @param listeners
   *   Number of listeners waiting after [[PartitionCache#get]] call. Too many of them might mean
   *   that cache is not being loaded fast enough.
   * @param entries
   *   Number of distinct journals stored in a cache. If it is too close to maximum configured
   *   number, the cache might not work efficiently.
   */
   ...
   *
   * Essentially it is just a wrapped [[Long]] value with a smart constructor and, therefore some
   * guaranteed properties, if constructor is used, i.e. value being non-negative.
   *
   * The main benefit of this class is to avoid mix up with other numerical values and to ensure
   * correct calculation from the two input numbers.
   *
   * Example:
   * {{{
   * scala> import cats.syntax.all.*
   * scala> import com.evolution.kafka.journal.PartitionCache.Diff
   *
   * scala> Diff.of(10, 20)
  ...
  object Diff {
     ...
     * Empty [[Diff]] value.
     *
     * It is used for [[CommutativeMonoid]] definition (i.e. to allow to combine [[Diff]] values)
     * and cannot be constructed using a smart constructor.
     */
   ...
   *
   * The `data` field might empty if Kafka record does not contain header with [[ActionHeader#key]].
   * Such records will be silently ignored by [[PartitionCache#add]] method.
   *
   * @param offset
   *   [[Offset]] of the record.
   * @param data
   *   Actual metainformation including journal identifier and the purpose of the record, i.e. if it
   *   is append, delete etc., but not including an event payload.
   */
  ...
  object Record {
     ...
     *   Journal identifier.
     * @param header
     *   Metainformation, including the purpose such as if this record appended events, deleted them
     *   or purged the journal.
     */
   ...
   *
   * @param offset
   *   [[Offset]] of _last_ (i.e. newest) non-replicated record related to this journal, which was
   *   also seen by [[PartitionCache]].
   * @param headInfo
   ...
   *
   * @param bounds
   *   Part of the Kafka topic partition containing non-replicated events. Corresponds to a first
   *   non-replicated offset and the last Kafka offset seen by [[PartitionCache]].
   * @param values
   *   Journal specific entries (i.e. the key is journal id). All offsets stored there are meant to
   *   be within `bounds` interval.
   */
