 ...
 * payload_bin BLOB
 * }}}
 * Where usage of either `payload_txt` or `payload_bin` column depends on the contents of a
 * `payload` field.
 *
 * The `payloadType` field is used to determine how the contents of `payload` should be treated,
 * i.e. if it should be parsed as JSON.
 */
