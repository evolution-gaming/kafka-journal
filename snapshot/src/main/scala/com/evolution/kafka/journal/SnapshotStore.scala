 ...
 *
 * Uses `Key` instead of `persistenceId`, similar to [[ReplicatedJournalFlat]] and
 * [[EventualJournal]].
 */
trait SnapshotStore[F[_]] {
   ...
   *
   * If several snapshots are found using a passed selection criteria for a specific key, then the
   * last one (i.e. with a latest `SeqNr`) is returned.
   *
   * @param key
   ...
   *
   * If several snapshots are found using a passed selection criteria for a specific key, then all
   * of them are deleted.
   *
   * @param key
   *   Unique identifier of a journal snapshot was done for.
   ...
   *
   * The method returns the same as `load(key, SnapshotSelectionCriteria.one(seqNr))`, but
   * additional optimizations might be possible.
   *
   * @param key
   *   Unique identifier of a journal snapshot was done for.
   * @param seqNr
