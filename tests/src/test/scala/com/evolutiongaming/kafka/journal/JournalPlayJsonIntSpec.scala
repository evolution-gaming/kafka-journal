...
class JournalPlayJsonIntSpec extends JournalIntSpec[Payload] {
  ...
  implicit override val kafkaRead: KafkaRead[IO, Payload]       = KafkaRead.payloadKafkaRead[IO]
  implicit override val kafkaWrite: KafkaWrite[IO, Payload]     = KafkaWrite.payloadKafkaWrite[IO]
  implicit override val eventualRead: EventualRead[IO, Payload] = EventualRead.payloadEventualRead[IO]
}
