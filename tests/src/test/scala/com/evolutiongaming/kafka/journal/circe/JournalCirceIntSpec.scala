...
class JournalCirceIntSpec extends JournalIntSpec[Json] {
  ...
  implicit override val kafkaRead: KafkaRead[IO, Json]       = Instances.kafkaRead[IO]
  implicit override val kafkaWrite: KafkaWrite[IO, Json]     = Instances.kafkaWrite[IO]
  implicit override val eventualRead: EventualRead[IO, Json] = Instances.eventualRead[IO]
}
