...
class ReplicatorIntSpec extends AsyncWordSpec with BeforeAndAfterAll with Matchers {
  ...
  implicit private val randomIdOf: RandomIdOf[IO] = RandomIdOf.uuid[IO]

  private def resources[F[
    _,
  ]: Async: LogOf: Parallel: FromFuture: ToFuture: RandomIdOf: MeasureDuration: FromTry: ToTry: Fail](
    ...
    def eventualJournal(conf: Config) = {
      ...
      for {
        ...
        eventualJournal <- EventualCassandra
          .of1[F](config, origin.some, none, cassandraClusterOf, DataIntegrityConfig.Default)
  ...
  "Replicator" should {
    ...
    def append(journal: Journal[IO], events: Nel[Event[Payload]], expireAfter: Option[ExpireAfter] = none) = {
      ...
      for {
        ...
        event <- events
      } yield EventRecord(event, timestamp, partitionOffset, origin.some, version.some, recordMetadata1, headers)
    }
    ...
    for {
      ...
      s"replicate append of $numberOfEvents events, seqNr: $seqNr" in {
        ...
        val result = for {
          ...
          } yield event(seqNr + n, Payload("kafka-journal"))
      ...
      for {
        ...
        s"consume event from kafka and replicate to eventual journal, seqNr: $seqNr, payload: $name" in {
          ...
          } yield offsetAfter should be > offsetBefore
