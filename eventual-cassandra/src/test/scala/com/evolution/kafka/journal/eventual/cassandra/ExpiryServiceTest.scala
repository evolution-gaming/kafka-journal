...
import com.evolution.kafka.journal.eventual.cassandra.ExpiryService.Action
import com.evolution.kafka.journal.util.MonadCancelFromMonad
...
class ExpiryServiceTest extends AnyFunSuite with Matchers {
  ...
  for {
    (expiry, expireAfter, action) <- List(
      ...
      (
        Expiry(1.day.toExpireAfter, LocalDate.of(2019, 12, 12).toExpireOn).some,
        1.day.toExpireAfter.some,
        Action.ignore,
      ),
