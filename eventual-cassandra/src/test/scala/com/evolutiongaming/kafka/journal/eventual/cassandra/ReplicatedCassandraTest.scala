  ...
  for {
          ...
          (
            ...
            Map(
              (
                ...
                MetaJournalEntry(
                  journalHead =
                    JournalHead(partitionOffset = partitionOffset, segmentSize = segmentSize, seqNr = SeqNr.min),
                  created = timestamp0,
                  updated = timestamp0,
                  origin = origin.some,
                ),
    ...
    test(s"append, $suffix") {
      ...
      val stateT = for {
        _ <- journal.append(
          ...
          events =
            Nel.of(eventRecordOf(seqNr = SeqNr.unsafe(1), partitionOffset = PartitionOffset(Partition.min, Offset.min))),
        )
        _ <- journal.append(
          key = key0,
          Partition.min,
      ...
      val actions = if (segmentSize <= SegmentSize.min) {
        List(
          ...
          Action
            .UpdateSeqNr(key0, segment0, PartitionOffset(Partition.min, Offset.unsafe(3)), timestamp1, SeqNr.unsafe(3)),
    ...
    test(s"not update expiry, $suffix") {
      ...
      val expected = State(
        actions = List(
          Action
            .UpdateSeqNr(key, segment, PartitionOffset(Partition.min, Offset.unsafe(2)), timestamp1, SeqNr.unsafe(2)),
          Action.InsertRecords(key, SegmentNr.min, 1),
          Action.InsertMetaJournal(
            key,
            segment,
    ...
    test(s"not repeat appends, $suffix") {
      ...
      val expected = State(
        actions = List(
          Action
            .UpdateSeqNr(key, segment, PartitionOffset(Partition.min, Offset.unsafe(4)), timestamp1, SeqNr.unsafe(2)),
          Action.InsertRecords(key, SegmentNr.min, 1),
        ),
        metaJournal = Map(
          (
    ...
    test(s"delete, $suffix") {
      ...
      val stateT = for {
        _ <- journal.append(
          ...
          expireAfter = none,
          events =
            Nel.of(eventRecordOf(seqNr = SeqNr.min, partitionOffset = PartitionOffset(Partition.min, Offset.min))),
        )
        _ <- journal.delete(
          key = key,
          Partition.min,
