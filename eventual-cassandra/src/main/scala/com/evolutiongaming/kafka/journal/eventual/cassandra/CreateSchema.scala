...
object CreateSchema {
  ...
  /** Creates Cassandra schema for eventual storage of a journal.
    *
    * The class does not perform a schema migration if any of the tables are already present in a database, and relies
    * on a caller to use a returned value to perfom the necessary migrations afterwards.
    *
    * @return
    *   Fully qualified table names, and `true` if all of the tables were created from scratch, or `false` if one or
    *   more of them were already present in a keyspace.
    */
