...
/** Cassandra-specific configuration used by a plugin.
  ...
  *   attempt.
  * @param segmentSize
  *   Number of records per one segment. The larger the number, the better the chance the recovery will hit the same
  *   partition, and be faster, but too large numbers are also will make larger Cassandra partitions with all the
