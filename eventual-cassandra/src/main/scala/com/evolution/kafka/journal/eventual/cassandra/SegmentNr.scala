 ...
 * ```
 *
 * In `journal` table [[SegmentNr]] is not limited but rather each segment contains not more than
 * [[SegmentSize]] events:
 * ```
 ...
 * ```
 *
 * Used to partition the `journal` table into segments thus distribute data between Cassandra
 * partitions. For `journal` table [[SegmentNr]] calculated based on [[SeqNr]] and [[SegmentSize]]
 * in [[SegmentNr.journal]]:
 * ```
 ...
 * ```
 *
 * Used to partition the `metajournal` table into segments thus simplify iterating over all records
 * in the table. For `metajournal` table [[SegmentNr]] calculated based on [[Key]] and [[Segments]]
 * in [[SegmentNr.metaJournal]]:
 * ```
 ...
 *
 * @see
 *   [[Segments]] for `metajournal` table
 * @see
 *   [[SegmentNrs]] for `metajournal` table
 * @see
 *   [[SegmentOf]] for `metajournal` table
 * @see
 *   [[SegmentNrsOf]] for `metajournal` table
 *
 * @see
 *   [[SegmentSize]] for `journal` table
 * @see
 *   [[Segment]] for `journal` table
 */
...
object SegmentNr {
  ...
  object Of {
     ...
     *
     * @param segments
     *   total number of segments in `metajournal` table
     * @return
     *   [[SegmentNr.Of]] factory
     */
