 ...
 *
 * This case class specifies configuration that could be set using `application.conf` (see
 * `reference.conf` for an example of such configuration).
 *
 * @param journal
 ...
 * @param startTimeout
 *   The timeout to create a journal adapter. Starting a journal involves some effectful steps, such
 *   as creating Cassandra session, so, in case of infrastructure or configuration troubles, it
 *   could take a longer time. Creating the journal will fail with [[TimeoutException]] if it takes
 *   longer than `startTimeout`.
 * @param stopTimeout
 *   This is meant to be a counterpart to `startTimeout`, allowing resource release to timeout with
 *   an error. This parameter is not used, for now, and `startTimeout` is used instead.
 * @param maxEventsInBatch
 *   Maximum number of events to write in one batch. I.e. if there is a request to write N >
 *   `maxEventsInBatch` events then the batch will be split to the writes of no more than
 *   `maxEventsInBatch` improving a latency and reducing the effect of potential failures.
 * @param callTimeThresholds
 *   Duration after a call to one of [[Journal]] methods will be reported as warning if it takes
 *   longer than the specified value. The calls to these methods are logged anyway, but on a debug
 *   logger level, which is, usually, switched off on production deployments.
 * @param jsonCodec
 *   JSON codec to use for (de)serialization of the events from [[scodec.bits.ByteVector]] to
 *   [[play.api.libs.json.JsValue]] and vice-versa. This parameter is only relevant if default
 *   [[KafkaJournal]] is used, i.e. it is not taken into account if Circe JSON or other custom
