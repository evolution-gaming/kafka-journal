...
class PayloadAndTypeSpec extends AnyFunSuite with Matchers with EitherValues {
  ...
  implicit private val fromAttempt: FromAttempt[Try]   = FromAttempt.lift[Try]
  implicit private val fromJsResult: FromJsResult[Try] = FromJsResult.lift[Try]
  ...
  for {
    ...
    (eventsName, eventsPlayJson, eventsCirceJson) <- List(
      ...
      (
        ...
        Events(
          Nel
            .of(event(1, Payload.json(PlayJson.obj("key" -> "1"))), event(2, Payload.json(PlayJson.obj("key" -> "2")))),
          metadata,
        ),
        Events(
          Nel.of(
