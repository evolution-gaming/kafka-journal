...
private[it] object SharedItEnv {
  ...
  private def runEnvResource: ResourceIO[SharedItEnv] = {
    logResourceStart(new ComponentLogger(componentName = "SharedItEnv")) {
      for {
  ...
  private def replicatorResource(config: ReplicatorConfig): ResourceIO[Unit] = {
    val replicatorLogger = new ComponentLogger(componentName = "Replicator")

    logResourceStart(replicatorLogger) {
      for {
  ...
  private def logResourceStart[T](componentLogger: ComponentLogger)(resource: ResourceIO[T]): ResourceIO[T] = {
    for {
      startTime <- IO {
        val startTime = Instant.now()
        componentLogger.info("starting...")
        startTime
      }.toResource
      result <- resource
      _ <- IO {
        val duration = java.time.Duration.between(startTime, Instant.now())
        componentLogger.info(s"started in $duration")
      }.toResource
    } yield result
  }
  ...
  private abstract class TestContainerDef[C <: GenericContainer[C]](
    ...
    private val componentLogger = new ComponentLogger(componentName = s"Container $name")

    val resource: ResourceIO[C] = {
      Resource.make(
        IO.blocking {
          val startTime = Instant.now()
          componentLogger.info(s"starting using image $image...")
          val container = makeContainer(image)
          container.start()
          val duration = java.time.Duration.between(startTime, Instant.now())
          componentLogger.info(s"started in $duration on ${ getConnString(container) }")
          container
        },
      ) { container =>
        IO.blocking {
          val startTime = Instant.now()
          componentLogger.info("stopping...")
          container.stop()
          val duration = java.time.Duration.between(startTime, Instant.now())
          componentLogger.info(s"stopped in $duration")
        }.onError {
          case t =>
            IO { componentLogger.error(s"failed to stop: ${ t.getMessage }", t) }
        }
      }
    }
  }
  ...
  /**
   * Convenience wrapper for logging lifecycle events of components of the shared integration test
   * environment (test containers, replicator, etc.)
   *
   * @param componentName
   *   component name - used to prefix log messages
   */
  private final class ComponentLogger(componentName: String) {
    private val prefix = s"[$componentName]"
