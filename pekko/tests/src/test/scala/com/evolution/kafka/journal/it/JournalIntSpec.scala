...
import com.evolution.kafka.journal.it.ItInstances.*
...
abstract class JournalIntSpec[A] extends AsyncFreeSpec with JournalSuite {
            ...
            "append duplicates" ignore {
              ...
              val seqNrs = {
                val seqNrs = (0 to 2).foldLeft(Nel.of(seqNr)) { (seqNrs, _) =>
                  seqNrs.head.next[Option].fold(seqNrs) {
                    _ :: seqNrs
                  }
                }
                seqNrs.reverse
