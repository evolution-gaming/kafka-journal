...
/**
 * Transforms persistent message between Pekko representation and internal Kafka Journal format.
 *
 * @tparam A
 *   Type of serialized payload. At the time of writing it could be either [[Payload]] by default,
 *   or [[Json]] if `kafka-journal-circe` module is used.
 */
trait EventSerializer[F[_], A] {

  /**
   * Transform persistent Pekko message to Kafka Journal specific internal representation.
   *
   * [[PersistentRepr#payload]] will get serialized to a form writable to Kafka and stored into
   * [[Event#payload]].
   *
   * The method may raise an error into `F[_]` if it is not possible to serialize a message, i.e.
   * for example if [[PersistentRepr#sequenceNr]] is negative or equals to zero.
   */
  def toEvent(persistentRepr: PersistentRepr): F[Event[A]]

  /**
   * Transform Kafka Journal event to persistent Pekko message.
   *
   * Parse [[Event#payload]] read from either Kafka or eventual store (i.e. Cassandra).
   *
   * The method may raise an error into `F[_]` if parsing fails, the event payload is not found or
...
object EventSerializer {
  ...
  /**
   * Single Pekko message before serialization to Kafka Journal event format.
   *
   * It represents three fields from [[PersistentRepr]] class, i.e. [[PersistentRepr#payload]],
   * [[PersistentRepr#writerUuid]] and [[PersistentRepr#manifest]].
   *
   * The reason this class was created is to make the logic constructing `Event[A]` in
   * [[EventSerializer]] generic by having ability to convert from an instance of
   * [[PersistentRepresentation]] to `A` and back.
   *
   ...
   * ```
   *
   * It might be possible to express the same logic without using the class, so in future it might
   * be removed as an overall simplification.
   *
   * It is usually serialized to a single `Payload` instance if Play JSON is used, or `Json`
   * instance if Circe is used instead.
   *
   * @param payload
   *   Non-serialized payload of an Pekko message/event. See also [[PersistentRepr#payload]].
   * @param writerUuid
   *   Same as [[PersistentRepr#writerUuid]]
   * @param manifest
   *   Same as [[PersistentRepr#manifest]], but the lack of manifest is represented by `None` rather
   *   than `""`.
   */
   ...
   *
   * This could be useful in unit tests, when serialization mechanism is not required.
   */
  ...
  /**
   * Serialize [[PersistentRepr]] to [[Payload]] using Pekko serialization or Play JSON.
   *
   * @see
   *   [[[#apply[F[_]](serializer*]]] for more details on how serialization mechanism is determined.
   */
   ...
   * Serialize [[PersistentRepr]] to [[Payload]] using Pekko serialization or Play JSON.
   *
   * The underlying type of a resulting [[Payload]] will depend on [[PersistentRepr#payload]] value.
   * I.e. it will use Play JSON and create [[Payload.Json]] if it is equal to [[JsValue]] and use
   * Pekko serialization and create [[Payload.Binary]] for everything else.
   *
   * `SerializedMsgSerializer` is only used if [[Payload.Binary]] is to be created or parsed. In
   * this case also `FromAttempt` parameter is used to append [[PersistentRepr#manifest]] and
   * [[PersistentRepr#writerUuid]] into a resulting [[ByteVector]].
   *
   * `FromJsResult` is used if [[Payload.Json]] is to be parsed. There is no need for a reverse
   * operation, because [[PersistentRepr#payload]] is already a [[JsValue]] in this case.
   *
   * @note
   *   It might be useful to have a unit test checking that [[Payload.Binary]] is not created
   *   accidentially, because it might be too easy to make a mistake while setting a value to
   *   [[PersistentRepr#payload]] (which has `Any` type and will accept anything assigned).
   */
   ...
   *
   * See `kafka-journal-circe` module for an example of how this method could be used.
   */
