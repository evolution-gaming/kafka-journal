...
/** Configuration for [[KafkaJournal]].
  ...
  *   Maximum number of events to write in one batch. I.e. if there is a request to write N > `maxEventsInBatch` events
  *   then the batch will be split to the writes of no more than `maxEventsInBatch` improving a latency and reducing the
  *   effect of potential failures.
  * @param callTimeThresholds
  *   Duration after a call to one of [[Journal]] methods will be reported as warning if it takes longer than the
  *   specified value. The calls to these methods are logged anyway, but on a debug logger level, which is, usually,
  ...
  *   [[play.api.libs.json.JsValue]] and vice-versa. This parameter is only relevant if default [[KafkaJournal]] is
...
object KafkaJournalConfig {
  ...
  implicit val configReaderKafkaJournalConfig: ConfigReader[KafkaJournalConfig] = {
    ...
    (cursor: ConfigCursor) =>
      ...
      } yield config.copy(journal = journal)
  }
