...
/** Transforms persistent message between Akka representation and internal Kafka Journal format.
  *
  * @tparam A
  *   Type of serialized payload. At the time of writing it could be either [[Payload]] by default, or [[Json]] if
  *   `kafka-journal-circe` module is used.
  */
...
object EventSerializer {
  ...
  /** Single Akka message before serialization to Kafka Journal event format.
    ...
    *
    * The reason this class was created is to make the logic constructing `Event[A]` in [[EventSerializer]] generic by
    * having ability to convert from an instance of [[PersistentRepresentation]] to `A` and back.
    *
    ...
    *
    * It is usually serialized to a single `Payload` instance if Play JSON is used, or `Json` instance if Circe is used
    * instead.
    *
    * @param payload
    ...
    * @param manifest
    *   Same as [[PersistentRepr#manifest]], but the lack of manifest is represented by `None` rather than `""`.
    */
  ...
  def of[F[_]: Sync: FromAttempt: FromJsResult](system: ActorSystem): F[EventSerializer[F, Payload]] =
    ...
    } yield apply[F](serializedMsgSerializer)

  /** Serialize [[PersistentRepr]] to [[Payload]] using Akka serialization or Play JSON.
    *
    * The underlying type of a resulting [[Payload]] will depend on [[PersistentRepr#payload]] value. I.e. it will use
    * Play JSON and create [[Payload.Json]] if it is equal to [[JsValue]] and use Akka serialization and create
    * [[Payload.Binary]] for everything else.
    *
    * `SerializedMsgSerializer` is only used if [[Payload.Binary]] is to be created or parsed. In this case also
    * `FromAttempt` parameter is used to append [[PersistentRepr#manifest]] and [[PersistentRepr#writerUuid]] into a
    * resulting [[ByteVector]].
    *
    * `FromJsResult` is used if [[Payload.Json]] is to be parsed. There is no need for a reverse operation, because
    * [[PersistentRepr#payload]] is already a [[JsValue]] in this case.
    *
    * @note
    *   It might be useful to have a unit test checking that [[Payload.Binary]] is not created accidentially, because it
    *   might be too easy to make a mistake while setting a value to [[PersistentRepr#payload]] (which has `Any` type
    *   and will accept anything assigned).
    */
  ...
  def apply[F[_]: MonadThrowable, A](
    ...
    override def toEvent(persistentRepr: PersistentRepr): F[Event[A]] = {
      ...
      } yield Event(seqNr, tagged.tags, payload.some)
