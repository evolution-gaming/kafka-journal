...
/** Main entry point to Kafka Journal implementation.
  *
  * The users are not expected to instantiate it directly, but should enable the plugin in respective `application.conf`
  * instead like this:
  * {{{
  * akka.persistence.journal.plugin = "evolutiongaming.kafka-journal.persistence.journal"
  * }}}
  *
  * This is achieved by having a special `reference.conf` file inside of the library JAR, which contains the required
  * configuration understandable by Akka Persistence.
  *
  ...
  *
  * In the cases, when the configuration provided by [[KafkaJournalConfig]] does not provide enough flexibility, it
  * might be useful to extend [[KafkaJournal]] itself and then override the necessary methods such as
  ...
  * }}}
  *
  * In this case the custom implementation could be used to replace the original class by adding the following to
  * `application.conf`:
  * {{{
  ...
  *
  * @param config
  *   Contains configuration coming from `application.conf` file, including [[KafkaJournalConfig]] parameters, and also
  *   selection of [[ToKey]] implementation. See the documentation for appropriate classes for more details.
  */
